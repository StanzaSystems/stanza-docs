...
Non-React user interfaces can get good value from the browser SDK, but if you are using React - use of the React SDK is recommended.

Stanza supports all evergreen browsers.

## Install the Browser library
```
npm install @stanzadev/browser
```
...
Stanza.init(config) // pass the config you defined earlier
```
