...
Stanza Next library makes it easier to keep values that need to be shared between client and server code in sync. Currently, we provide utilities to keep [`enablementNumber`](../concepts#enablement-number) consistent between the client-server boundary.
