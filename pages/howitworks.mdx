...
At heart, Stanza is what SREs would call a traffic management tool. 
...
- `Stanza SDKs` run in server-side code, typically as middleware. They read request metadata and use it to prioritize requests under load. They also emit basic telemetry to Stanza to help detect problems before the user does.
